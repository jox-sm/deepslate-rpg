...
Organize and link the likes system inside `app/api/games/[id]` so that it processes actions through **Redis Queues**.

## 2. Centralizing Redis Queues
Eliminate all individual Redis queue setups. The code has blown up significantly because we keep creating a new queue for every single component. Instead, we'll turn it into a **generic utility** that takes parameters like the query name or queue name. This gives us a **centralized logic** and makes the code much more readable.

## 3. State Synchronization (Myers' Algorithm vs. Event-Driven)
We need to analyze and look into using **Myers' algorithm** to compare JSON data, extract mutations, and merge them. We can actually modify JSON objects directly via Upstash without uploading the entire file. So, replace GraphQL with Myers' algorithm just to extract the specific parts that changed.

For example, if a character is added, we can handle this in two ways: **Event-Driven** or **Myers' Diff**.

**Myers' Diff** is a far better choice because the processing happens entirely on the **client-side (user's device)** before hitting the server. On the other hand, the **Event-Driven** approach relies fully on the server, which introduces higher latency and consumes server processing power.

Consequently, if 10,000 concurrent users are online sending messages while interacting with the AI, going Event-Driven would be reckless since we are on the **Vercel Free Tier**. We would be heavily restricted by the number of operations we can run. Plus, doing it this way brings user latency down to near-zero.

Alternatively, maintaining a local variable on the client to track mutations and ship them instantly instead of running Myers' algorithm altogether. This seems like a better fix at first glance, but here is why **Redis** is vastly more efficient: the total lines of code won't even exceed 500 lines anyway, yet a user can continuously modify the exact same item (whether it's a character, a map, or anything else).
...
**Wait for user to:**
- Add `@upstash/redis` package + env vars
...
1. **`POST /api/push` handler for `type: 'like'`:**
   ...
   - Add direct Upstash Redis write: `INCRBY likes:{gameId} {delta}` using the Upstash Redis client
...
1. **`types/patches.ts`**: JSON Patch types scoped to game data
   ...
   type GamePatchRequest = {
     id: string
     patches: GamePatch[]
     lastSavedSnapshot?: GameDataJSON
   }
   ```
...
1. **`components/adventures/cards/like-button.tsx`**
   ...
   - On 200: `successToast(...)` → optimistic increment
   - On error: `errorToast(...)` → revert optimistic count
...
2. **`lib/mutation-backup.ts`**
   - `backupPatches(gameId, patches)` → localStorage
   - `restorePatches(gameId)` → returns saved patches or null
   - `clearBackup(gameId)` → removes key
   - Auto-backup via `setInterval(save, 600_000)`

3. **Save-on-exit prompt** via `window.onbeforeunload` when `dirty`

---

## Step 6: Upgrade to `@upstash/redis` SDK

- Run: `npm install @upstash/redis`
- Add env vars: `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN`
- Replace `lib/queue.ts` ioredis singleton with `new Redis({ url, token })`

---

## Step 6 Follow-up: Unified to Upstash Redis (Bug Fix)

**Root cause:** The dual-client setup (`redis` = Upstash, `legacyRedis` = ioredis) meant `lib/cache-warmup.ts` was writing to **Redis Cloud** while `app/api/games/route.ts` was reading from **Upstash**. Two different Redis databases — no shared data — so `getCachedGameIds()` always returned an empty array and no cards rendered.

**Fix applied (2026-06-06):**

1. `lib/queue.ts` — ioredis moved to comment block, only `redis` (Upstash) exported.
2. `lib/cache-warmup.ts` — switched from `legacyRedis` to `redis`; replaced `pipeline().exec()` with `Promise.all()`.
3. `utilities/hotnessCache.ts` — switched import; `checkMemoryPressure()` returns `false` (Upstash REST API doesn't expose INFO/MEMORY).
4. `utilities/gameFetchPipeline.ts` — switched import; replaced `pipeline().exec()` with `Promise.all()`.
...
**Verdict:** Upstash is massively better for serverless (Vercel Free Tier). The limitations (no INFO, no pub/sub, no blocking ops) don't affect this project's use case.
