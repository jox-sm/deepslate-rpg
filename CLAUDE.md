...
- **Design System:** `app/globals.css` @theme block — Stone/Slate + Torchlight palette: charcoal/slate base (`--color-charcoal-950` → `--color-stone-dust-200`), torchlight accent (`--color-torch-600` → `--color-torch-200`, accent `--color-torch-400`), gold (`--color-gold-*`) and blood (`--color-blood-*`) legendary/danger tones, runic-silver text (`--color-text-primary`), semantic tokens (`--color-bg-surface`, `--color-text-primary`, `--color-accent`), glass utilities, glow shadows, gradient text utilities.
...
- **Cards Grid:** CSS Columns layout (`column-count: 3/2/1`), gradient card backgrounds, compact design (image 3:2 → name → tags with gradient bg).
...
### Backend
- Convex real-time tables (games, characters, maps, items) — opt-in realtime scaffold; `requireAuth()`/`requireStaff()` guards.
- Clerk auth with JWT validation (`validateJWTMiddleware(request)`); Supabase server client via `lib/auth.ts` (custom Neon/Mongo JWT templates removed).
- API routes with idempotency (UUID v7 + Upstash Redis dedup, TTL 300s).
- Cache layer: Upstash Redis (games list, individual game, metadata, job queue).
- DB: Neon PostgreSQL (primary game data) + MongoDB (extended data: characters, maps, items).
- Image uploads: Supabase Storage with sharp WebP (quality 80), retry, progress tracking, binary conversion.
- Background worker for game creation pipeline (Upstash Redis queue → DB insert → cache warm).

### Key Conventions
- DB retries: `async () => { await sql... }` wrapper (not direct `sql` call).
- `cacheInitialized` flag sets only after successful warm-up attempt.
- Supabase storage hostname + quality 80 (sharp WebP in `utilities/imagesUtils.ts`).
